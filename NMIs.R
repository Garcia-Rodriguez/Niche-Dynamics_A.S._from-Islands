...
setwd("C:/Users/Pichi/Dropbox/PROJECTS_VIENNA/CLIMATIC DYNAMICS/AHORASI/")
...
NMIs.amph<-pblapply(1:3, function (x)
{
  ...
  alien.scores<-na.omit(suprow(pca,terra::extract(clim,alien[,4:5]))$li)
  ...
  mypath <- file.path("RESULTS/NMI/Plots",
                      paste("NMI_", clust.amph$Spp_Country[x],
                            ".pdf", sep = ""))
  pdf(file=mypath)
  # plot axes
  plot(coordinates(bkg.pts),type="n",xlab="PC1",ylab="PC2",main=clust.amph$Spp_Country[x])
  ...
  plot(outer,col=Pal.outer(100),add=T, legend=FALSE, main=clust.amph$Spp_Country[x])
...
NMIs.amph2<-do.call(rbind,NMIs.amph)
NMIs.amph2$class<-"Amphibia"
print(NMIs.amph2)
